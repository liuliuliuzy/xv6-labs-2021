...
ialloc(uint dev, short type)
{
    ...
    for (inum = 1; inum < sb.ninodes; inum++)
    {
        ...
        // 根据块起始地址与dinode结构体的大小，可以找到inum对应的dinode位置，将其存储到指针dip中
        dip = (struct dinode *)bp->data + inum % IPB;
        if (dip->type == 0)
        { // a free inode
            memset(dip, 0, sizeof(*dip));
            dip->type = type;
            // 因为这里修改过bp->data，所以需要调用log_write()，将操作记录到log中。
...
void iupdate(struct inode *ip)
{
    ...
    // 因为修改了磁盘中的内容，所以需要log_write()记录下这一操作
    log_write(bp);
    brelse(bp);
}
...
int readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
    ...
    if (off > ip->size || off + n < off) // 如果偏移量大于文件大小，或者读取数量n为负数，函数返回
        return 0;
    if (off + n > ip->size)
        n = ip->size - off;

    for (tot = 0; tot < n; tot += m, off += m, dst += m) // 更新循环变量部分存在多个操作的for语句
    {
...
//
static char *
skipelem(char *path, char *name) // 纯字符串处理函数
{
    char *s;
    int len;
...
namex(char *path, int nameiparent, char *name)
{
    ...
    {
        ilock(ip);
        if (ip->type != T_DIR) // 当前inode对应的文件类型一定要是DIR类型
        {
            iunlockput(ip);
            return 0;
        }
